...
- **✨ Gemini AI 智慧行銷文案大師 & BYOK 安全機制**：
  - 自動依據 App 定位生成 ASO 吸睛標題、副標題與故事線（Story Arc），並支援一鍵英/日/繁中多國在地化翻譯。
  - 支援 **智慧 ASO 離線備援引擎 (0 元免 Key)** 與 **自備 Gemini API Key (BYOK)**。
  - 🔒 **端對端隱私保證**：密鑰直連 Google 官方端點，絕不上傳或儲存於任何後端伺服器。
  - ⚠️ **使用者端 3 大潛在風險防範指引**：
    1. 🔴 **公用電腦遭他人調用**：在公共設備請選「a. 不記住 (Session)」或離席前按垃圾桶清除。
    2. 🔴 **螢幕肉眼旁觀窺探**：系統預設密碼點點遮罩（••••）並提供自動縮略遮罩（`AIzaSy...****`）。
    3. 🔴 **惡意瀏覽器擴充外掛**：建議以無痕模式執行，或直接使用「c. 智慧 ASO 離線引擎（零風險）」。
- **📦 一鍵官方規格 ZIP 批次打包**：
